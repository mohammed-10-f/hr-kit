...
function ratingMarkup(r){const avg=Number(r.rating_average||0),count=Number(r.rating_count||0);return `<div class="rating" data-rating-id="${r.id}"><span class="rating-stars" aria-label="متوسط التقييم ${avg.toFixed(1)} من 5">${[1,2,3,4,5].map(i=>`<button type="button" class="rating-star ${i<=Math.round(avg)?'active':''}" data-rate="${i}" aria-label="تقييم ${i} من 5">★</button>`).join('')}</span><small>${avg?avg.toFixed(1):'—'} (${count.toLocaleString('ar-SA')})</small></div>`}
async function submitRating(id,rating,root){try{const d=await getJSON('/api/resources/'+id+'/rating',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rating,visitor_id:visitorId()})});root.querySelectorAll('.rating-star').forEach((b,i)=>b.classList.toggle('active',i<Math.round(d.rating_average)));root.querySelector('small').textContent=d.rating_average.toFixed(1)+' ('+Number(d.rating_count).toLocaleString('ar-SA')+')';root.classList.add('rated')}catch(e){if(root)root.title=e.message||'تعذر حفظ التقييم'}}
function bindRatings(){document.querySelectorAll('[data-rating-id]').forEach(root=>{root.querySelectorAll('.rating-star').forEach(b=>b.addEventListener('click',()=>submitRating(Number(root.dataset.ratingId),Number(b.dataset.rate),root)))})}
...
async function loadResources(){
 ...
 $('#resources-list').innerHTML=lastResources.map(r=>publicList(r,q)).join('');bindRatings();trackViews();
 $('#empty').classList.toggle('hidden',lastResources.length>0);
 const showFeatured=!q&&!activeCategory;
 if(showFeatured){const featured=await getJSON('/api/resources?featured=1');$('#featured').innerHTML=featured.map(r=>resourceCard(r,'')).join('');bindRatings();trackViews();$('#featured-section').classList.toggle('hidden',featured.length===0)}else $('#featured-section').classList.add('hidden');
 firstLoad=false;
}
