...
let activeCategory="",allCategories=[],filterType="",filterFeatured=false,lastResources=[],firstLoad=true;
...
function matchesType(r){if(!filterType)return true;const [,cls]=fileInfo(r);return cls===filterType}

function updateFiltersCount(){const n=(filterType?1:0)+(filterFeatured?1:0);const badge=$('#filters-count');if(n>0){badge.hidden=false;badge.textContent=n}else badge.hidden=true}

function resourceCard(r,q){const [type,cls]=fileInfo(r);return `<article class="resource-card"><div class="file-icon ${cls}">${fileIconMark(cls)}</div><div class="resource-top"><span class="tag">${esc(r.category_names||'عام')}</span>${r.featured?`<span class="featured-mark" title="ملف مختار" aria-label="ملف مختار">${iconSvg('star')}</span>`:''}</div><h3>${highlight(r.title,q)}</h3><p>${highlight(r.description||'ملف من مرجع الموارد البشرية.',q)}</p><div class="meta">الإصدار ${esc(r.version||'1.0')} · ${Number(r.downloads||0).toLocaleString('ar-SA')} تحميل</div><a class="download" href="/api/download/${encodeURIComponent(r.id)}"><span>تحميل الملف</span><span class="download-icon">${iconSvg('download')}</span></a></article>`}

function publicList(r,q){const [type,cls]=fileInfo(r);return `<article class="public-list-item"><div class="file-icon ${cls}">${fileIconMark(cls)}</div><div class="public-list-main"><strong>${highlight(r.title,q)}</strong><small>${esc(r.category_names||'عام')} · الإصدار ${esc(r.version||'1.0')}</small></div><span class="public-list-date">${r.updated_at?new Date(r.updated_at.replace(' ','T')+'Z').toLocaleDateString('ar-SA'):''}</span><a class="list-download" href="/api/download/${encodeURIComponent(r.id)}" aria-label="تحميل ${esc(r.title)}"><span>تحميل</span><span class="download-icon">${iconSvg('download')}</span></a></article>`}

async function loadResources(){
 const q=$('#search').value.trim();
 if(firstLoad){$('#resources-list').innerHTML=skeletonList(6);$('#featured').innerHTML=skeletonCards(4)}
 const url=`/api/resources?q=${encodeURIComponent(q)}&category=${encodeURIComponent(activeCategory)}${filterFeatured?'&featured=1':''}`;
 const data=await getJSON(url);
 lastResources=data.filter(matchesType);
 $('#results-title').textContent=(q||activeCategory||filterType||filterFeatured)?'نتائج البحث':'أحدث الملفات';
 $('#results-count').textContent=lastResources.length?`${lastResources.length.toLocaleString('ar-SA')} ملف`:'تحديثات المرجع الأخيرة';
 $('#resources-list').innerHTML=lastResources.map(r=>publicList(r,q)).join('');
 $('#empty').classList.toggle('hidden',lastResources.length>0);
 const showFeatured=!q&&!activeCategory&&!filterType&&!filterFeatured;
 if(showFeatured){const featured=await getJSON('/api/resources?featured=1');$('#featured').innerHTML=featured.map(r=>resourceCard(r,'')).join('');$('#featured-section').classList.toggle('hidden',featured.length===0)}else $('#featured-section').classList.add('hidden');
 firstLoad=false;
}

function openFilters(){$('#filters-panel').classList.add('open');$('#filters-toggle').setAttribute('aria-expanded','true');document.body.classList.add('no-scroll')}
function closeFilters(){$('#filters-panel').classList.remove('open');$('#filters-toggle').setAttribute('aria-expanded','false');document.body.classList.remove('no-scroll')}
...
$('#clear-filter').onclick=()=>{$('#search').value='';activeCategory='';filterType='';filterFeatured=false;$('#filter-type').value='';$('#filter-featured').checked=false;updateFiltersCount();loadCategories();loadResources();window.scrollTo({top:0,behavior:'smooth'})};
$('#filter-type').addEventListener('change',e=>{filterType=e.target.value;updateFiltersCount();loadResources()});
$('#filter-featured').addEventListener('change',e=>{filterFeatured=e.target.checked;updateFiltersCount();loadResources()});
$('#filters-toggle').addEventListener('click',()=>{$('#filters-panel').classList.contains('open')?closeFilters():openFilters()});
$('#filters-close').addEventListener('click',closeFilters);
$('#filters-apply').addEventListener('click',closeFilters);
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeFilters();closeNav()}});
