...
export default {async fetch(request,env){
 ...
 try{await ensureSchema(env);
  ...
  if(url.pathname==='/api/resources'&&request.method==='GET'){
   const q=normalizeArabic(url.searchParams.get('q')||''),cat=url.searchParams.get('category')||'',featured=url.searchParams.get('featured')||'';let sql=`SELECT r.id,r.title,r.slug,r.description,r.file_url,r.file_name,r.file_type,r.keywords,r.version,r.downloads,r.views,r.featured,r.created_at,r.updated_at, GROUP_CONCAT(DISTINCT c.name) category_names, GROUP_CONCAT(DISTINCT c.slug) category_slugs FROM resources r LEFT JOIN resource_categories rc ON rc.resource_id=r.id LEFT JOIN categories c ON c.id=rc.category_id AND c.is_visible=1 WHERE r.status='published'`;
  ...
  if(url.pathname.startsWith('/api/resources/')&&request.method==='POST'&&url.pathname.endsWith('/reaction')){const id=Number(url.pathname.split('/')[3]);const b=await request.json();const reaction=String(b.reaction||'').trim();const visitor=String(b.visitor_id||'').trim();if(!visitor||!['like','dislike'].includes(reaction))return cors(bad('بيانات التقييم غير صحيحة'));const item=await env.DB.prepare("SELECT id FROM resources WHERE id=? AND status='published'").bind(id).first();if(!item)return cors(bad('File not found',404));const vh=await sha256(visitor);const existing=await env.DB.prepare('SELECT reaction FROM resource_reactions WHERE resource_id=? AND visitor_hash=?').bind(id,vh).first();if(existing?.reaction===reaction){await env.DB.prepare('DELETE FROM resource_reactions WHERE resource_id=? AND visitor_hash=?').bind(id,vh).run()}else{await env.DB.prepare('INSERT INTO resource_reactions(resource_id,visitor_hash,reaction) VALUES(?,?,?) ON CONFLICT(resource_id,visitor_hash) DO UPDATE SET reaction=excluded.reaction,updated_at=CURRENT_TIMESTAMP').bind(id,vh,reaction).run()}const stats=await env.DB.prepare("SELECT COUNT(CASE WHEN reaction='like' THEN 1 END) likes,COUNT(CASE WHEN reaction='dislike' THEN 1 END) dislikes FROM resource_reactions WHERE resource_id=?").bind(id).first();const mine=await env.DB.prepare('SELECT reaction FROM resource_reactions WHERE resource_id=? AND visitor_hash=?').bind(id,vh).first();return cors(json({ok:true,likes:Number(stats?.likes||0),dislikes:Number(stats?.dislikes||0),my_reaction:mine?.reaction||''}))}
