...
export default {async fetch(request,env){
  ...
  try{
    ...
    if(url.pathname==="/api/resources"&&request.method==="GET"){
      ...
      let sql=`SELECT r.id,r.title,r.slug,r.description,r.file_url,r.file_name,r.file_type,r.keywords,r.version,r.downloads,r.featured,r.created_at,r.updated_at FROM resources r WHERE r.status='published' AND EXISTS (SELECT 1 FROM resource_categories rc JOIN categories c ON c.id=rc.category_id WHERE rc.resource_id=r.id AND c.is_visible=1)`;
      const b=[];if(q){sql+=" AND (r.title LIKE ? OR r.description LIKE ? OR r.keywords LIKE ? OR r.file_name LIKE ?)";const x=`%${q}%`;b.push(x,x,x,x)}if(cat){sql+=" AND EXISTS (SELECT 1 FROM resource_categories rc JOIN categories c ON c.id=rc.category_id WHERE rc.resource_id=r.id AND c.slug=? AND c.is_visible=1)";b.push(cat)}if(featured==="1")sql+=" AND r.featured=1";sql+=featured==="1"?" ORDER BY r.downloads DESC,r.updated_at DESC LIMIT 6":" ORDER BY r.updated_at DESC LIMIT 100";
      const {results}=await env.DB.prepare(sql).bind(...b).all();
      for(const r of results){const cats=await env.DB.prepare(`SELECT c.id,c.name,c.slug,c.icon FROM resource_categories rc JOIN categories c ON c.id=rc.category_id WHERE rc.resource_id=? AND c.is_visible=1 ORDER BY c.sort_order,c.name`).bind(r.id).all();r.categories=cats.results||[];r.category_name=r.categories[0]?.name||'';r.category_slug=r.categories[0]?.slug||'';r.icon=r.categories[0]?.icon||'📄';}
      return cors(json(results));
    }
    ...
    if(catMatch&&request.method==="DELETE")return admin(request,env,async()=>{const id=Number(catMatch[1]),n=await env.DB.prepare("SELECT COUNT(*) n FROM resource_categories rc JOIN resources r ON r.id=rc.resource_id WHERE rc.category_id=? AND r.status!='archived'").bind(id).first();if(Number(n?.n)>0)return bad("لا يمكن حذف قسم يحتوي على ملفات. انقل الملفات أولًا.");await env.DB.prepare("DELETE FROM categories WHERE id=?").bind(id).run();return json({ok:true})});

    if(url.pathname==="/api/admin/resources"&&request.method==="GET")return admin(request,env,async()=>{const {results}=await env.DB.prepare(`SELECT r.* FROM resources r ORDER BY CASE r.status WHEN 'published' THEN 1 WHEN 'hidden' THEN 2 ELSE 3 END,r.updated_at DESC`).all();for(const r of results){const cats=await env.DB.prepare(`SELECT c.id,c.name,c.slug,c.icon FROM resource_categories rc JOIN categories c ON c.id=rc.category_id WHERE rc.resource_id=? ORDER BY c.sort_order,c.name`).bind(r.id).all();r.categories=cats.results||[];r.category_ids=r.categories.map(c=>c.id);r.category_name=r.categories[0]?.name||'';r.icon=r.categories[0]?.icon||'📄';}return json(results)});
    if(url.pathname==="/api/admin/resources"&&request.method==="POST")return admin(request,env,async()=>{const b=await request.json(),title=String(b.title||"").trim(),fileUrl=String(b.file_url||"").trim();if(!title||!fileUrl)return bad("اسم الملف ورابطه مطلوبان");const categoryIds=[...(Array.isArray(b.category_ids)?b.category_ids:[]),...(b.category_id?[b.category_id]:[])].map(Number).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i);if(!categoryIds.length)return bad("اختر قسمًا واحدًا على الأقل");const slug=slugify(title)+"-"+Date.now(),fileName=String(b.file_name||inferFileName(fileUrl)),fileType=String(b.file_type||inferFileType(fileName));const r=await env.DB.prepare(`INSERT INTO resources(title,slug,description,category_id,file_url,file_name,file_type,keywords,version,status,featured,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`).bind(title,slug,b.description||"",categoryIds[0],fileUrl,fileName,fileType,b.keywords||"",b.version||"1.0","published",b.featured?1:0).run();const id=r.meta.last_row_id;await env.DB.batch(categoryIds.map(cid=>env.DB.prepare("INSERT OR IGNORE INTO resource_categories(resource_id,category_id) VALUES(?,?)").bind(id,cid)));return json({ok:true,id},201)});
    const resMatch=url.pathname.match(/^\/api\/admin\/resources\/(\d+)$/);
    if(resMatch&&request.method==="PATCH")return admin(request,env,async()=>{const id=Number(resMatch[1]),b=await request.json(),allowed={title:"title",description:"description",file_url:"file_url",keywords:"keywords",version:"version",status:"status",featured:"featured"},sets=[],vals=[];for(const k in allowed)if(b[k]!==undefined){sets.push(`${allowed[k]}=?`);vals.push(k==='featured'?(b[k]?1:0):b[k])}if(b.file_url!==undefined){const n=inferFileName(String(b.file_url));if(n){sets.push("file_name=?");vals.push(n);sets.push("file_type=?");vals.push(inferFileType(n))}}if(b.category_ids!==undefined){const categoryIds=Array.isArray(b.category_ids)?b.category_ids.map(Number).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i):[];if(!categoryIds.length)return bad("اختر قسمًا واحدًا على الأقل");sets.push("category_id=?");vals.push(categoryIds[0]);await env.DB.prepare("DELETE FROM resource_categories WHERE resource_id=?").bind(id).run();await env.DB.batch(categoryIds.map(cid=>env.DB.prepare("INSERT OR IGNORE INTO resource_categories(resource_id,category_id) VALUES(?,?)").bind(id,cid)))}if(b.category_id!==undefined&&!b.category_ids){sets.push("category_id=?");vals.push(Number(b.category_id)||null);await env.DB.prepare("DELETE FROM resource_categories WHERE resource_id=?").bind(id).run();if(b.category_id)await env.DB.prepare("INSERT OR IGNORE INTO resource_categories(resource_id,category_id) VALUES(?,?)").bind(id,Number(b.category_id)).run()}if(!sets.length)return bad("لا توجد تغييرات");sets.push("updated_at=CURRENT_TIMESTAMP");vals.push(id);await env.DB.prepare(`UPDATE resources SET ${sets.join(",")} WHERE id=?`).bind(...vals).run();return json({ok:true})});
