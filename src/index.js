...
function hydrateResources(rows){return (rows||[]).map(r=>({...r,category_ids:r.category_ids?String(r.category_ids).split(',').filter(Boolean).map(Number):[],category_names:r.category_names||'',rating_average:Number(r.rating_average||0),rating_count:Number(r.rating_count||0),like_count:Number(r.like_count||0),dislike_count:Number(r.dislike_count||0),views:Number(r.views||0),downloads:Number(r.downloads||0)}))}
...
export default {async fetch(request,env){
 ...
 try{await ensureSchema(env);
  ...
  if(url.pathname==='/api/resources'&&request.method==='GET'){
   const q=normalizeArabic(url.searchParams.get('q')||''),cat=url.searchParams.get('category')||'',featured=url.searchParams.get('featured')||'';let sql=`SELECT r.id,r.title,r.slug,r.description,r.file_url,r.file_name,r.file_type,r.keywords,r.version,r.downloads,r.views,r.featured,r.created_at,r.updated_at, (SELECT COUNT(*) FROM resource_reactions rr1 WHERE rr1.resource_id=r.id AND rr1.reaction='like') like_count, (SELECT COUNT(*) FROM resource_reactions rr2 WHERE rr2.resource_id=r.id AND rr2.reaction='dislike') dislike_count, GROUP_CONCAT(DISTINCT c.name) category_names, GROUP_CONCAT(DISTINCT c.slug) category_slugs FROM resources r LEFT JOIN resource_categories rc ON rc.resource_id=r.id LEFT JOIN categories c ON c.id=rc.category_id AND c.is_visible=1 WHERE r.status='published'`;
  ...
  if(url.pathname.startsWith('/api/resources/')&&request.method==='POST'&&url.pathname.endsWith('/reaction')){const id=Number(url.pathname.split('/')[3]);const b=await request.json();const reaction=String(b.reaction||'').trim();const visitor=String(b.visitor_id||'').trim();if(!visitor||!['like','dislike'].includes(reaction))return cors(bad('بيانات التفاعل غير صحيحة'));const item=await env.DB.prepare("SELECT id FROM resources WHERE id=? AND status='published'").bind(id).first();if(!item)return cors(bad('File not found',404));const vh=await sha256(visitor);const existing=await env.DB.prepare('SELECT reaction FROM resource_reactions WHERE resource_id=? AND visitor_hash=?').bind(id,vh).first();if(existing?.reaction===reaction){await env.DB.prepare('DELETE FROM resource_reactions WHERE resource_id=? AND visitor_hash=?').bind(id,vh).run()}else{await env.DB.prepare('INSERT INTO resource_reactions(resource_id,visitor_hash,reaction) VALUES(?,?,?) ON CONFLICT(resource_id,visitor_hash) DO UPDATE SET reaction=excluded.reaction,created_at=CURRENT_TIMESTAMP').bind(id,vh,reaction).run()}const stats=await env.DB.prepare("SELECT COALESCE(SUM(CASE WHEN reaction='like' THEN 1 ELSE 0 END),0) like_count,COALESCE(SUM(CASE WHEN reaction='dislike' THEN 1 ELSE 0 END),0) dislike_count FROM resource_reactions WHERE resource_id=?").bind(id).first();const current=await env.DB.prepare('SELECT reaction FROM resource_reactions WHERE resource_id=? AND visitor_hash=?').bind(id,vh).first();return cors(json({ok:true,reaction:current?.reaction||null,like_count:Number(stats?.like_count||0),dislike_count:Number(stats?.dislike_count||0)}))}
  ...
  if(url.pathname==='/api/admin/resources'&&request.method==='GET')return admin(request,env,async()=>{const {results}=await env.DB.prepare(`SELECT r.*,GROUP_CONCAT(DISTINCT c.name) category_names,GROUP_CONCAT(DISTINCT c.id) category_ids,COALESCE(AVG(rr.rating),0) rating_average,COUNT(DISTINCT rr.rowid) rating_count,(SELECT COUNT(*) FROM resource_reactions rx1 WHERE rx1.resource_id=r.id AND rx1.reaction='like') like_count,(SELECT COUNT(*) FROM resource_reactions rx2 WHERE rx2.resource_id=r.id AND rx2.reaction='dislike') dislike_count FROM resources r LEFT JOIN resource_categories rc ON rc.resource_id=r.id LEFT JOIN categories c ON c.id=rc.category_id LEFT JOIN resource_ratings rr ON rr.resource_id=r.id GROUP BY r.id ORDER BY CASE r.status WHEN 'published' THEN 1 WHEN 'hidden' THEN 2 ELSE 3 END,r.updated_at DESC`).all();return json(hydrateResources(results))});
