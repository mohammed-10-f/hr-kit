...
function hydrateResources(rows){return (rows||[]).map(r=>({...r,category_ids:r.category_ids?String(r.category_ids).split(',').filter(Boolean).map(Number):[],category_names:r.category_names||'',rating_average:Number(r.rating_average||0),rating_count:Number(r.rating_count||0),views:Number(r.views||0),downloads:Number(r.downloads||0)}))}
...
export default {async fetch(request,env){
 ...
 try{await ensureSchema(env);
  ...
  if(url.pathname==='/api/resources'&&request.method==='GET'){
   const q=normalizeArabic(url.searchParams.get('q')||''),cat=url.searchParams.get('category')||'',featured=url.searchParams.get('featured')||'';let sql=`SELECT r.id,r.title,r.slug,r.description,r.file_url,r.file_name,r.file_type,r.keywords,r.version,r.downloads,r.featured,r.created_at,r.updated_at, GROUP_CONCAT(DISTINCT c.name) category_names, GROUP_CONCAT(DISTINCT c.slug) category_slugs FROM resources r LEFT JOIN resource_categories rc ON rc.resource_id=r.id LEFT JOIN categories c ON c.id=rc.category_id AND c.is_visible=1 WHERE r.status='published'`;
  ...
  if(url.pathname.startsWith('/api/resources/')&&request.method==='POST'&&url.pathname.endsWith('/rating')){const id=Number(url.pathname.split('/')[3]);const b=await request.json();const rating=Math.round(Number(b.rating));const visitor=String(b.visitor_id||'').trim();if(!visitor||rating<1||rating>5)return cors(bad('بيانات التقييم غير صحيحة'));const item=await env.DB.prepare("SELECT id FROM resources WHERE id=? AND status='published'").bind(id).first();if(!item)return cors(bad('File not found',404));const vh=await sha256(visitor);try{await env.DB.prepare('INSERT INTO resource_ratings(resource_id,visitor_hash,rating) VALUES(?,?,?)').bind(id,vh,rating).run()}catch(e){if(String(e.message||'').includes('UNIQUE'))return cors(bad('سبق لك تقييم هذا الملف.',409));throw e}const stats=await env.DB.prepare('SELECT COALESCE(AVG(rating),0) rating_average,COUNT(*) rating_count FROM resource_ratings WHERE resource_id=?').bind(id).first();return cors(json({ok:true,rating_average:Number(stats?.rating_average||0),rating_count:Number(stats?.rating_count||0)}))}
