...
- لا توجد كلمة مرور افتراضية داخل الكود. عند تهيئة قاعدة بيانات جديدة يمكن تعيين `ADMIN_INITIAL_PASSWORD` كـ Cloudflare Secret لمرة التهيئة الأولى.
- تسجيل دخول لوحة الإدارة محمي بحد للمحاولات: 5 محاولات فاشلة خلال 15 دقيقة لكل عنوان مصدر.
- كلمات المرور الجديدة تستخدم PBKDF2-SHA-256 مع salt، مع ترقية تلقائية لكلمة المرور القديمة عند أول تسجيل دخول ناجح.
- تم تقييد CORS للـ same-origin وإضافة Security Headers أساسية.
- تم منع رفع الامتدادات التنفيذية/السكريبتية الخطرة، مع الإبقاء على رفع ملفات الموارد المعتادة مثل PDF وWord وExcel وPowerPoint وZIP والصور.
- الروابط الخارجية للملفات تقبل HTTP/HTTPS فقط.

## محرك PDF المحلي — مهم

ميزة النماذج الإلكترونية لا تعتمد على CDN لتشغيل PDF.js أو PDF-Lib.
عند تنفيذ `npm install` يتم نسخ مكتبات PDF.js وPDF-Lib إلى `public/vendor`، ثم يقوم `npm run deploy` بالتحقق من وجودها قبل النشر.
...
لا تستخدم `npx wrangler deploy` مباشرة إذا كانت نسخة `public/vendor` غير موجودة؛ استخدم `npm run deploy` حتى يتم تجهيز محرك PDF المحلي والتحقق منه أولًا.
